...
describe("TaskManager", () => {
  ...
  beforeAll(async () => {
    module = await Test.createTestingModule({ imports: [TestTypeORMModule] }).compile();
  ...
  it("should activate task", async () => {
    const queuedTasks = await uow.Task.FilterTasks(null, null, TaskStatuses.Queued, null);
  ...
  it("should not activate task because task is already activated", async () => {
    const activeTasks = await uow.Task.FilterTasks(null, null, TaskStatuses.Active, null);
    const employees = await uow.Employee.GetAllEmployees();
    const taskManager = new TaskManager();
  ...
  it("should cancel task", async () => {
    ...
    const manager = employees.find((employee) => employee.Role == Roles.Manager);
    const errorMessage = taskManager.CancelTask(employee, manager, TaskCancelCauses.LackOfSpace);
  ...
  it("should not cancel task because task is not active", async () => {
    ...
    const employees = await uow.Employee.GetAllEmployees();
    const taskManager = new TaskManager();
    const employee = employees.find((employee) => employee.Role == Roles.Employee);
    const task = queuedTasks[0];
  ...
  it("should not cancel task because task is not cancelled by a manager", async () => {
    ...
    const employee = employees.find((employee) => employee.ActiveTask);
    const task = employee.ActiveTask;
    const manager = employees.find((employee) => employee.Role == Roles.Employee);
    const errorMessage = taskManager.CancelTask(employee, manager, TaskCancelCauses.LackOfSpace);
  ...
  it("should not cancel task because arguments are missing", async () => {
    ...
    expect(taskManager.CancelTask(null, manager, TaskCancelCauses.LackOfSpace)).not.toBeNull();
    expect(taskManager.CancelTask(employee, null, TaskCancelCauses.LackOfSpace)).not.toBeNull();
  ...
  it("should finish task", async () => {
    ...
    const taskManager = new TaskManager();
    const employee = employees.find(
      (employee) => employee.ActiveTask && employee.ActiveTask.TaskDetails.IsCompleted()
    );
    const task = employee.ActiveTask;
    const hall = halls.find((hall) => hall.Tasks.some((t) => _.isEqual(t, task)));
  ...
  it("should not finish task, because employee doesn't have an active task", async () => {
    ...
    const taskManager = new TaskManager();
    const employee = employees.find((employee) => !employee.ActiveTask);
    const hall = halls.find((hall) => hall.Employees.some((e) => _.isEqual(e, employee)));
    const errorMessage = taskManager.FinishTask(employee, hall, await uow.Product.GetAllProducts());
  ...
  it("should not finish task, because hall is improper", async () => {
    ...
    const task = employee.ActiveTask;
    const hall = halls.find((hall) => hall.Tasks.every((t) => !_.isEqual(t, task)));
    const products = await uow.Product.FilterProducts(
      null,
      null,
      null,
  ...
  it("should not finish task, because the task manager did not receive all products from task", async () => {
    ...
    const errorMessage = taskManager.FinishTask(employee, hall, products.splice(0, 1));
