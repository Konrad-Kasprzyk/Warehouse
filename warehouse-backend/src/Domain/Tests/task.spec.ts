...
describe("Task", () => {
  ...
  it("should be defined", () => {
    ...
    let taskDetails = new TaskDetails(1, startShelf, new Shelf(2, "gtin2", ShelfPurposes.Storage));
  ...
  it("should have activation, finish and cancel time set to null", async () => {
    let queuedTask = (await uow.Task.FilterTasks(null, null, TaskStatuses.Queued, null))[0];
  ...
  it("should have activation and cancel time set", async () => {
    let cancelledTask = (await uow.Task.FilterTasks(null, null, TaskStatuses.Cancelled, null))[0];
  ...
  it("should have activation and finish time set", async () => {
    let finishedTask = (await uow.Task.FilterTasks(null, null, TaskStatuses.Finished, null))[0];
  ...
  it("should cancel task", async () => {
    let activeTask = (await uow.Task.FilterTasks(null, null, TaskStatuses.Active, null))[0];
    ...
    expect(activeTask.TaskCancelCause).toEqual(TaskCancelCauses.ProductsMissing);
  });

  it("should finish task", async () => {
    let activeTask = (await uow.Task.FilterTasks(null, null, TaskStatuses.Active, null)).find(
      (task) => task.TaskDetails.IsCompleted()
    );
    activeTask.Finish();
