...
describe("TaskDetails", () => {
  ...
  it("should be defined", () => {
    ...
    startShelf.AddProduct(new Product(new ProductModel("partNumber", "brand", "name"), "gtin"));
  ...
  it("should throw error", () => {
    ...
    const destinationShelf = new Shelf(2, "gtin2", ShelfPurposes.Storage);
    const destinationShelfImproperProductModel = new Shelf(3, "gtin3", ShelfPurposes.Storage);
    destinationShelfImproperProductModel.AddProduct(
      new Product(new ProductModel("partNumber2", "brand2", "name2"), "gtin4")
    );

    expect(() => new TaskDetails(-1, startShelf, destinationShelf)).toThrowError();
    expect(() => new TaskDetails(null, startShelf, destinationShelf)).toThrowError();
    expect(() => new TaskDetails(1, emptyStartShelf, destinationShelf)).toThrowError();
    expect(
  ...
  it("should accept shelf and product scan", async () => {
    ...
    const taskDetails = activeTaskDetails.find(
      (taskDetails) =>
        taskDetails.PickedUpProductsGtins.length > 0 && taskDetails.StoredProductsGtins.length == 0
    );
    const destinationShelf = await uow.Hall.GetShelf(taskDetails.DestinationShelfGtin);
    const productToStore = await uow.Product.GetProduct(taskDetails.PickedUpProductsGtins[0]);
  ...
  it("should reject product scan because it will be too much scanned products", async () => {
    ...
    const productModels = await uow.ProductModel.GetAllProductModels();
    const taskDetails = activeTaskDetails.find(
      (taskDetails) => taskDetails.PickedUpProductsGtins.length == taskDetails.ScansRequired
    );
    const startingShelf = await uow.Hall.GetShelf(taskDetails.StartingShelfGtin);
    const productModel = productModels.find(
      (p) =>
        p.Brand == taskDetails.ProductModelBrand &&
        p.PartNumber == taskDetails.ProductModelPartNumber
    );
    const products = await uow.Product.FilterProducts(null, null, productModel, null);
    ...
    expect(taskDetails.PickedUpProductsGtins).toHaveLength(taskDetails.ScansRequired);
  });
  ...
  it("should return that task is completed", async () => {
    ...
    const taskDetails = activeTaskDetails.find(
      (taskDetails) => taskDetails.StoredProductsGtins.length == taskDetails.ScansRequired
    );
