...
export class ProductModelController {
  ...
  async getOne(@Param("id") id: number): Promise<ProductModel> {
    ...
    if (!productModel)
      throw new HttpException(`Product model with id ${id} not found`, HttpStatus.NOT_FOUND);
    return productModel;
  }
  ...
  async addProductModel(
    ...
    try {
      return await this.productModelService.AddProductModel(body.name, body.brand, body.partNumber);
    } catch (err) {
  ...
  @Post("delete")
  async removeProductModel(@Body() body: { id: number }): Promise<ProductModel> {
    try {
