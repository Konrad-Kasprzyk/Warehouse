...
export class TaskService {
  ...
  AddTask(
    ...
    return this.uow.InTransaction(async (uow: IUnitOfWork) => {
      ...
      const startingShelf = hall.Shelves.find(
        (shelf) => shelf.Gtin == startingShelfGtin,
      );
      if (!startingShelf)
      ...
      const destinationShelf = hall.Shelves.find(
        (shelf) => shelf.Gtin == destinationShelfGtin,
      );
      if (!destinationShelf)
        return Promise.reject(
  ...
  async FilterTasks(
    ...
    return this.uow.Task.FilterTasks(
      hall,
      employee,
      castedTaskStatus,
      activationDate,
  ...
  ScanProduct(taskId: number, productGtin: string): Promise<Task> {
    return this.uow.InTransaction(async (uow: IUnitOfWork) => {
      ...
      if (!product)
        return Promise.reject(`Product with GTIN ${productGtin} not found`);
      if (!task.ScanProduct(product))
        return Promise.reject(`Product scan rejected`);
  ...
  ActivateTask(taskId: number, assignedEmployeeId: number): Promise<Task> {
    return this.uow.InTransaction(async (uow: IUnitOfWork) => {
      ...
      if (!employee)
        return Promise.reject(
          `Employee with id ${assignedEmployeeId} not found`,
        );
  ...
  FinishTask(employeeId: number, hallNumber: number): Promise<Task> {
    return this.uow.InTransaction(async (uow: IUnitOfWork) => {
      ...
      const errorMessage = this.taskManager.FinishTask(
        employee,
        hall,
        taskProducts,
      );
  ...
  CancelTask(
    ...
    return this.uow.InTransaction(async (uow: IUnitOfWork) => {
      ...
      if (!manager)
        return Promise.reject(`Manager with id ${managerId} not found`);
      const castedTaskCancelCause = TaskCancelCauses[taskCancelCause];
      if (!castedTaskCancelCause)
        return Promise.reject('Invalid task cancel cause');
      const cancelledTask = employee.ActiveTask;
      const errorMessage = this.taskManager.CancelTask(
        employee,
        manager,
        castedTaskCancelCause,
      );
      if (errorMessage) return Promise.reject(errorMessage);
