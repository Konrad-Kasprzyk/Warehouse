...
export class ProductService {
  ...
  async FilterProducts(
    ...
    return this.uow.Product.FilterProducts(hall, shelf, productModel, productGtins);
  }
  ...
  AddProduct(
    ...
    return this.uow.InTransaction(async (uow: IUnitOfWork) => {
      ...
      const productModel = await uow.ProductModel.GetProductModel(productModelId);
      if (!productModel) return Promise.reject(`Product model with id ${productModelId} not found`);

      const product = await uow.Product.AddProduct(new Product(productModel, productGtin));
      if (!shelf.AddProduct(product))
        return Promise.reject(`Improper shelf. Can't add this product to shelf`);
