...
export class HallService {
  ...
  MoveEmployeeToAnotherHall(employeeId: number, newHallNumber: number): Promise<Hall> {
    return this.uow.InTransaction(async (uow: IUnitOfWork) => {
      const newHall = await uow.Hall.GetHall(newHallNumber);
      if (!newHall) return Promise.reject(`Hall with number ${newHallNumber} not found`);
      ...
      const index = currentHall.Employees.findIndex((employee) => employee.id == employeeId);
      if (index < 0) return Promise.reject(`Employee with id ${employeeId} not found in hall`);
  ...
  async FilterShelves(
    ...
    if (productModelId) {
      productModel = await this.uow.ProductModel.GetProductModel(productModelId);
      if (!productModel) return Promise.reject(`Product model with id ${productModelId} not found`);
    }
    if (shelfPurpose) {
  ...
  AddShelf(
    ...
    return this.uow.InTransaction(async (uow: IUnitOfWork) => {
      ...
      if (!hall) return Promise.reject(`Hall with number ${hallNumber} not found`);
      const shelf = await uow.Hall.AddShelf(new Shelf(shelfNumber, gtin, castedShelfPurpose));
