...
describe("ProductModelService", () => {
  ...
  it("should add product model", async () => {
    const productModel = await productModelService.AddProductModel("name", "brand", "part number");
  ...
  it("should not add product model because arguments are missing", async () => {
    ...
    await expect(productModelService.AddProductModel("name", "brand", null)).rejects.toBeDefined();
  });
});
