...
describe("HallService", () => {
  ...
  beforeAll(async () => {
    ...
    productModelService = await module.resolve<ProductModelService>(ProductModelService);
  });
  ...
  it("should not move employee to hall, because employee already is there", async () => {
    await expect(hallService.MoveEmployeeToAnotherHall(1, 1)).rejects.toBeDefined();
  });

  it("should not move employee to hall, because employee is not found", async () => {
    await expect(hallService.MoveEmployeeToAnotherHall(142, 1)).rejects.toBeDefined();
  });

  it("should not move employee to hall, because hall is not found", async () => {
    await expect(hallService.MoveEmployeeToAnotherHall(1, 42)).rejects.toBeDefined();
  });

  it("should not move employee to hall, because arguments are improper", async () => {
    await expect(hallService.MoveEmployeeToAnotherHall(-42, 1)).rejects.toBeDefined();
    await expect(hallService.MoveEmployeeToAnotherHall(1, -42)).rejects.toBeDefined();
  });

  it("should not move employee to hall, because arguments are missing", async () => {
    await expect(hallService.MoveEmployeeToAnotherHall(null, 1)).rejects.toBeDefined();
    await expect(hallService.MoveEmployeeToAnotherHall(1, null)).rejects.toBeDefined();
  });
  ...
  it("should return an array of all shelves from one hall", async () => {
    ...
    hall.Shelves.forEach((shelf) => expect(shelves.some((e) => _.isEqual(e, shelf))).toBeTruthy());
  });
  ...
  it("should return an array of all shelves with same product model", async () => {
    ...
    shelves.forEach((shelf) => expect(shelf.ProductModelBrand).toEqual(productModel.Brand));
  ...
  it("should return an array of all shelves with same shelf purpose", async () => {
    const shelves = await hallService.FilterShelves(null, 1, ShelfPurposes.Delivery);

    shelves.forEach((shelf) => expect(shelf.ShelfPurpose).toBe(ShelfPurposes.Delivery));
  });
  ...
  it("should add shelf", async () => {
    const shelf = await hallService.AddShelf(1, 42, "gtin42", ShelfPurposes.Delivery);
  ...
  it("should not add shelf because arguments are missing", async () => {
    ...
    await expect(hallService.AddShelf(1, 42, null, ShelfPurposes.Delivery)).rejects.toBeDefined();
    await expect(hallService.AddShelf(1, 42, "gtin42", null)).rejects.toBeDefined();
  });

  it("should not add shelf because GTINs will duplicate", async () => {
    const shelf = (await hallService.FilterShelves(1, null, null))[0];

    await expect(hallService.AddShelf(1, 42, shelf.Gtin, shelf.ShelfPurpose)).rejects.toBeDefined();
  });
});
