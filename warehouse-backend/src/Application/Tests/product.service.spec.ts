...
describe("ProductService", () => {
  ...
  it("should return an array of all products from one hall", async () => {
    ...
    const filteredProducts = await productService.FilterProducts(1, null, null, null);
  ...
  it("should return an array of all products from one shelf", async () => {
    ...
    const filteredProducts = await productService.FilterProducts(null, shelf.Gtin, null, null);
  ...
  it("should return an array of all products with same product number", async () => {
    ...
    const filteredProducts = await productService.FilterProducts(null, null, 1, null);
  ...
  it("should return an array of concrete products based on GTIN", async () => {
    ...
    const filteredProducts = await productService.FilterProducts(null, null, null, productGtins);

    productGtins.forEach((gtin) =>
      expect(filteredProducts.some((product) => product.Gtin == gtin)).toBeTruthy()
    );
  });
  ...
  it("should not add product because arguments are improper", async () => {
    ...
    await expect(productService.AddProduct(-42, shelf.Gtin, 1, "gtin42")).rejects.toBeDefined();
    await expect(
      productService.AddProduct(1, "not existing shelf GTIN", 1, "gtin42")
    ).rejects.toBeDefined();
    await expect(productService.AddProduct(1, shelf.Gtin, -42, "gtin42")).rejects.toBeDefined();
  });

  it("should not add product because arguments are missing", async () => {
    const shelf = (await hallService.GetAllShelves())[0];

    await expect(productService.AddProduct(null, shelf.Gtin, 1, "gtin42")).rejects.toBeDefined();
    await expect(productService.AddProduct(1, null, 1, "gtin42")).rejects.toBeDefined();
    await expect(productService.AddProduct(1, shelf.Gtin, null, "gtin42")).rejects.toBeDefined();
    await expect(productService.AddProduct(1, shelf.Gtin, 1, null)).rejects.toBeDefined();
  });

  it("should not add product because GTINs will duplicate", async () => {
    const shelf = (await hallService.FilterShelves(1, null, null))[0];
    const product = (await productService.FilterProducts(1, shelf.Gtin, null, null))[0];
