...
describe("TaskService", () => {
  ...
  it("should add task from delivery shelf to storage shelf", async () => {
    const deliveryShelves = await hallService.FilterShelves(1, null, ShelfPurposes.Delivery);
    const storageShelves = await hallService.FilterShelves(1, null, ShelfPurposes.Storage);
    const deliveryShelf = deliveryShelves.find((deliveryShelf) =>
      storageShelves.some(
        (storageShelf) =>
          storageShelf.ProductModelBrand == deliveryShelf.ProductModelBrand &&
          storageShelf.ProductModelPartNumber == deliveryShelf.ProductModelPartNumber
      )
    );
    ...
    const task = await taskService.AddTask(1, deliveryShelf.Gtin, storageShelf.Gtin, 1);
  ...
  it("should add task from storage shelf to shipment shelf", async () => {
    const storageShelves = await hallService.FilterShelves(1, null, ShelfPurposes.Storage);
    const shipmentShelves = await hallService.FilterShelves(1, null, ShelfPurposes.Shipment);
    const storageShelf = storageShelves.find((storageShelf) =>
      shipmentShelves.some(
        (shipmentShelf) =>
          ...
          shipmentShelf.ProductModelPartNumber == storageShelf.ProductModelPartNumber
      )
    );
    ...
    const task = await taskService.AddTask(1, storageShelf.Gtin, shipmentShelf.Gtin, 1);
  ...
  it("should add task from delivery shelf to shipment shelf", async () => {
    ...
    const deliveryShelf = deliveryShelves.find((deliveryShelf) =>
      shipmentShelves.some(
        (shipmentShelf) =>
          ...
          shipmentShelf.ProductModelPartNumber == deliveryShelf.ProductModelPartNumber
      )
    );
    const shipmentShelf = shipmentShelves.find(
      (s) =>
    ...
    const task = await taskService.AddTask(1, deliveryShelf.Gtin, shipmentShelf.Gtin, 1);
  ...
  it("should add task from delivery shelf to empty shipment shelf", async () => {
    const startingShelf = (await hallService.FilterShelves(1, null, ShelfPurposes.Delivery))[0];
    const shipmentShelves = await hallService.FilterShelves(1, null, ShelfPurposes.Shipment);
    const emptyShipmentShelf = await shipmentShelves.find((shelf) => !shelf.ProductModelBrand);
    const task = await taskService.AddTask(1, startingShelf.Gtin, emptyShipmentShelf.Gtin, 1);

    expect(task).toBeDefined();
    expect(task.TaskDetails.StartingShelfGtin).toEqual(startingShelf.Gtin);
    expect(task.TaskDetails.DestinationShelfGtin).toEqual(emptyShipmentShelf.Gtin);
    expect(task.TaskDetails.ScansRequired).toEqual(1);
  });

  it("should add task from storage shelf to empty shipment shelf", async () => {
    const startingShelf = (await hallService.FilterShelves(1, null, ShelfPurposes.Storage))[0];
    const shipmentShelves = await hallService.FilterShelves(1, null, ShelfPurposes.Shipment);
    const emptyShipmentShelf = await shipmentShelves.find((shelf) => !shelf.ProductModelBrand);
    const task = await taskService.AddTask(1, startingShelf.Gtin, emptyShipmentShelf.Gtin, 1);
  ...
  it("should not add task because shelves are from different hall", async () => {
    const storageShelves = await hallService.FilterShelves(1, null, ShelfPurposes.Storage);
    const shipmentShelves = await hallService.FilterShelves(2, null, ShelfPurposes.Shipment);
    const storageShelf = storageShelves.find((storageShelf) =>
      shipmentShelves.some(
        (shipmentShelf) =>
          shipmentShelf.ProductModelBrand == storageShelf.ProductModelBrand &&
  ...
  it("should not add task because the starting shelf has different product model than the destination shelf", async () => {
    const startingShelves = await hallService.FilterShelves(1, null, ShelfPurposes.Delivery);
    const destinationShelves = await hallService.FilterShelves(1, null, ShelfPurposes.Storage);
  ...
  it("should not add task because arguments are missing", async () => {
    ...
    await expect(taskService.AddTask(1, null, storageShelf.Gtin, 1)).rejects.toBeDefined();
    await expect(taskService.AddTask(1, deliveryShelf.Gtin, null, 1)).rejects.toBeDefined();
    await expect(
  ...
  it("should not add task because arguments are improper", async () => {
    ...
    await expect(taskService.AddTask(1, "wrong GTIN", storageShelf.Gtin, 1)).rejects.toBeDefined();
    await expect(taskService.AddTask(1, deliveryShelf.Gtin, "wrong GTIN", 1)).rejects.toBeDefined();
    await expect(
  ...
  it("should return an array of all tasks from one hall", async () => {
    ...
    hall.Tasks.forEach((task) => expect(tasks.some((t) => _.isEqual(t, task))).toBeTruthy());
  });
  ...
  it("should return an array of all tasks assigned to one employee", async () => {
    ...
    employee.Tasks.forEach((task) => expect(tasks.some((t) => _.isEqual(t, task))).toBeTruthy());
  });

  it("should return an array of all tasks with one task status", async () => {
    const tasks = await taskService.FilterTasks(null, null, TaskStatuses.Queued, null);

    expect(tasks).toBeDefined();
    expect(tasks.length).toBeGreaterThan(1);
    tasks.forEach((task) => expect(task.Status == TaskStatuses.Queued).toBeTruthy());
  });

  it("should return an array of all tasks activated on the same day", async () => {
    const activationDate = new Date(2022, 0, 25, 1);
    const tasks = await taskService.FilterTasks(null, null, null, activationDate);

    expect(tasks).toBeDefined();
    expect(tasks.length).toBeGreaterThan(1);
    tasks.forEach((task) => {
      task.ActiveTime.setHours(1, 0, 0, 0);
      expect(task.ActiveTime.getTime() == activationDate.getTime()).toBeTruthy();
    });
  });

  it("should return an array of all cancelled tasks by a manager", async () => {
    const manager = (await employeeService.FilterEmployees(Roles.Manager, null)).find(
      (manager) => manager.CancelledTasks?.length > 0
    );
    const tasks = await taskService.FilterTasks(null, manager.id, TaskStatuses.Cancelled, null);

    expect(tasks).toBeDefined();
    expect(tasks.length).toBeGreaterThan(0);
    tasks.forEach((task) => expect(task.Status == TaskStatuses.Cancelled).toBeTruthy());
  });

  it("should accept product scan", async () => {
    let task = (await taskService.FilterTasks(null, null, TaskStatuses.Active, null)).find(
      (task) =>
        !task.TaskDetails.IsCompleted() &&
        task.TaskDetails.PickedUpProductsGtins?.length > 0 &&
        task.TaskDetails.ScannedShelfGtin == task.TaskDetails.DestinationShelfGtin
    );
  ...
  it("should not accept product scan, because proper shelf was not scanned.", async () => {
    ...
    await expect(taskService.ScanProduct(task.id, scannedProductGtin)).rejects.toBeDefined();
  });
  ...
  it("should not accept product scan, because product has an improper product model", async () => {
    ...
    const product = products.find(
      (product) =>
        ...
        product.ProductModel.PartNumber != task.TaskDetails.ProductModelPartNumber
    );

    await expect(taskService.ScanProduct(task.id, product.Gtin)).rejects.toBeDefined();
  });

  it("should not accept product scan, because task is not activated", async () => {
    let task = (await taskService.FilterTasks(null, null, TaskStatuses.Queued, null))[0];
    const products = await productService.GetAllProducts();
    const product = products.find(
      (product) =>
        product.ProductModel.Brand != task.TaskDetails.ProductModelBrand &&
  ...
  it("should accept shelf scan", async () => {
    let task = (await taskService.FilterTasks(null, null, TaskStatuses.Active, null)).find(
      (task) => task.TaskDetails.ScannedShelfGtin != task.TaskDetails.DestinationShelfGtin
    );
    task = await taskService.ScanShelf(task.id, task.TaskDetails.DestinationShelfGtin);

    expect(task).toBeDefined();
    expect(task.TaskDetails.ScannedShelfGtin).toEqual(task.TaskDetails.DestinationShelfGtin);
  });

  it("should not accept shelf scan, because shelf has an improper product model.", async () => {
    let task = (await taskService.FilterTasks(null, null, TaskStatuses.Active, null))[0];
    ...
    await expect(taskService.ScanShelf(task.id, shelf.Gtin)).rejects.toBeDefined();
  });

  it("should not accept shelf scan, because task is not activated", async () => {
    const task = (await taskService.FilterTasks(null, null, TaskStatuses.Queued, null))[0];
  ...
  it("should not activate task, because task is already activated", async () => {
    ...
    await expect(taskService.ActivateTask(task.id, employee.id)).rejects.toBeDefined();
  });
  ...
  it("should not activate task, because task is finished", async () => {
    ...
    let task = (await taskService.FilterTasks(null, null, TaskStatuses.Finished, null))[0];
  ...
  it("should not activate task, because arguments are improper", async () => {
    ...
    await expect(taskService.ActivateTask(-42, employee.id)).rejects.toBeDefined();
  ...
  it("should not activate task, because arguments are missing", async () => {
    ...
    await expect(taskService.ActivateTask(null, employee.id)).rejects.toBeDefined();
  ...
  it("should not finish task, because task is not completed", async () => {
    ...
    await expect(taskService.FinishTask(employee.id, hall.Number)).rejects.toBeDefined();
  });
  ...
  it("should not finish task, because arguments are improper", async () => {
    ...
    await expect(taskService.FinishTask(-42, hall.Number)).rejects.toBeDefined();
    await expect(taskService.FinishTask(employee.id, -42)).rejects.toBeDefined();
  });
  ...
  it("should not finish task, because arguments are missing", async () => {
    ...
    await expect(taskService.FinishTask(null, hall.Number)).rejects.toBeDefined();
    await expect(taskService.FinishTask(employee.id, null)).rejects.toBeDefined();
  });
  ...
  it("should not cancel task, because employee doesn't have an active task", async () => {
    ...
    await expect(
      taskService.CancelTask(employee.id, managers[0].id, TaskCancelCauses.LackOfSpace)
    ).rejects.toBeDefined();
  });
  ...
  it("should not cancel task, because normal employee tries to cancel task", async () => {
    ...
    const secondEmployee = employees.find((employee) => employee.Name != firstEmployee.Name);

    await expect(
      taskService.CancelTask(firstEmployee.id, secondEmployee.id, TaskCancelCauses.LackOfSpace)
    ).rejects.toBeDefined();
  });
  ...
  it("should not cancel task, because arguments are improper", async () => {
    ...
    await expect(
      taskService.CancelTask(employee.id, managers[0].id, "wrong task cancel cause")
    ).rejects.toBeDefined();
  });
  ...
  it("should not cancel task, because arguments are missing", async () => {
    ...
    await expect(
      taskService.CancelTask(null, managers[0].id, TaskCancelCauses.LackOfSpace)
    ).rejects.toBeDefined();
    await expect(
      taskService.CancelTask(employee.id, null, TaskCancelCauses.LackOfSpace)
    ).rejects.toBeDefined();
    await expect(taskService.CancelTask(employee.id, managers[0].id, null)).rejects.toBeDefined();
  });
});
